...
#define	BADID	-3
#define	ERROR	-4
char	conerr[NCONERR][8] = {"BUSY","TIMEOUT","BAD ID","ERROR"};
...
char	phone_id[3] = PHONE_ID;	/*PHONE_ID must be defined in header file gcos.h*/
...
int	n;
{
	...
	if(snsum > MAXJOB)
		mvbaddf();
	trouble("write err: ioc =%6o, pid %.5s",
		ioc, dfname+LPID);
}
