...
switch(t)
	...
	case STWHILE:
		q->nextlab = thislab();
		if(vp)
			exifgo( mknode(TNOTOP,OPNOT,vp,0),
				q->breaklab = nextlab() );
...
{
ptr p, q;
int i, first, deflabno, blab, cmin, cmax, range, caseval, optcase;
int labp[101];
...
/*		if(thisexec->brnchend == 0)	*/
		...
		range = cmax - cmin + 1;
		if(optcase && ncases>2 && range<2*ncases && range<100)
			{
			for(i=0; i<range ; ++i)
				labp[i] = 0;
			for(p=thisctl->loopctl ; p!=0 ; p = p->nextcase)
	...
	default:
		fatal1("popctl: invalid control block type %d", thisctl->subtype);
	}
