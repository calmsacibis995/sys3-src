...
if(p->tag != TCALL)
	fatal1("bad tag %d in excall", p->tag);
...
if(p)
	{
	if(procname && procname->vtype)
		{
		if(p->tag!=TNAME || p->sthead!=procname->sthead)
			exasgn( cpexpr(procname) , OPASGN, p);
		}
	else execerr("can only return values in a function", 0);
	}
...
if( (tag = e->tag)==TERROR || e->vtype!=TYLOG)
	...
	if(tag != TERROR)
		execerr("non-logical conditional expression in if", 0);
	}
...
putic(ICMARK,0);
putic(ICOP,OPLPAR);
prexpr(e = simple(RVAL, mknode(TNOTOP,OPNOT,e,0)));
putic(ICOP,OPRPAR);
putic(ICMARK,0);
afterif = 1;
frexpr(e);
...
levskip = ( levp==0 ? 1 : convci(levp->leftp) );
if(levskip <= 0)
	{
...
for(i = 0; i<3 ; ++i)
	...
	if(q = p->dopar[i])
		...
		if(isicon(p->dopar[i], &val[i]))
			{
			if(val[i] <= 0)
				execerr("do parameter out of range", 0);
			}
		else	val[i] = -1;
		}
	}
...
if(val[0]>0 && val[1]>0 && val[0]>val[1])
	execerr("do parameters out of order", 0);
return(p);
}
