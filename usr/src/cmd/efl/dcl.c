...
if(c1<'a' || c2<c1 || c2>'z')
	dclerr("bad implicit range", 0);
else if(type==TYUNDEFINED || type>TYLCOMPLEX)
	dclerr("bad type in implicit statement", 0);
else
	for(i = c1 ; i<=c2 ; ++i)
		impltype[i-'a'] = type;
}
...
if(e->tag!=TLIST && e->tag!=TREPOP)
	e = mknode(TREPOP, NULL, arrsize(v), e);
...
v = cpexpr(v);
if(v->vsubs == 0)
	v->vsubs = mknode(TLIST,0, mkchain(mkint(1),0), 0);
