...
{
int qval;
qval = q && ( equals(q, "yes") || equals(q, "on") );
...
if(equals(p,"debug")) dbgopt = 1;
	...
	tailor.lngcxtype = qval;
else if(equals(p, "longcomplexprefix"))
	tailor.lngcxprefix = qval;
...
if(langopt==2) setdot(dotsopt);
else if(langopt==1) setdot(1);
}
...
{
int val;
struct table { char *optn; ptr optloc; } *p;
static struct table formats[ ] =  {
