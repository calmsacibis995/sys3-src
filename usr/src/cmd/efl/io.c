...
if(unit->tag!=TCONST && (unit->tag!=TNAME || unit->vsubs!=0))
	unit = simple(LVAL, mknode(TASGNOP,OPASGN,gent(TYINT,0),unit));
...
if(errhandle && tailor.errmode==IOERRIBM)
	...
	if(endlabel)
		...
		exasgn(cpexpr(errval), OPASGN,
			mknode(TNEGOP,OPMINUS,mkint(1),0) );
		}
	exlab(jumplabel);
	}
...
for(p = list ; p ; p = p->nextp)
	...
	switch( (q = p->datap) ->tag)
		...
		default:
			fatal1("doiolist: impossible block tag %d", q->tag);
		}
	}
}

...
for(p = list; p; p = p->nextp)
	...
	switch ( (q = p->datap) ->tag)
		...
		default:
			fatal1("impossible block tag %d", q->tag);
		}
	if(q->iofmt)
		cfree(q->iofmt);
	cfree(q);
...
for( ; p ; p = p->nextp)
	switch( (q = p->datap) ->tag )
		...
		default:
			fatal1("impossible ioblock tag %d", q->tag);
		}
}
...
q = simple(LVAL, q);
if(q->vsubs == NULL)
	q->vsubs = mknode(TLIST,0, mkchain(cpexpr(v),NULL), NULL);
else
	q->vsubs->leftp->datap = simple(SUBVAL, mknode(TAROP,OPPLUS, cpexpr(v),
		     mknode(TAROP,OPMINUS,q->vsubs->leftp->datap,mkint(1))));
q->vdim = NULL;
gp = mkiogroup( mkchain(mkioitem(q,NULL),NULL), p->iofmt, dop);
gp->nrep = nrep;
cfree(p);
return(gp);
}

mkformat(letter, n1, n2)
char letter;
register ptr n1, n2;
{
char f[20], *fp, *s;
...
if(errcheck)
	...
	putic(ICOP, OPCOMMA);
	putsii(ICCONST, "iostat =");
	prexpr(cpexpr( t = gent(TYINT,NULL)));
	putic(ICOP, OPRPAR);
	return( t );
	}
else	{
