...
switch(t)
	...
	case TRELOP:
		...
		if(lt==TYCHAR || rt==TYCHAR)
			...
			if(l->vtype != r->vtype)
				{
				exprerr("comparison of character and noncharacter data",0);
				goto err;
				}
			ll = conval(l->vtypep);
			rl = conval(r->vtypep);
			...
			exprerr("relational involving logicals", 0);
		else if( (lt==TYCOMPLEX || rt==TYCOMPLEX) &&
			o!=OPEQ && o!=OPNE)
				exprerr("order comparison of complex numbers", 0);
		else if(lt != rt)
			{
			if(lt==TYINT)
				p->leftp = coerce(rt, l);
	...
	case TLOGOP:
		...
		if(r->vtype != TYLOG)
			{
			exprerr("non-logical operand of logical operator",0);
			goto err;
			}
	case TNOTOP:
		ckdcl(l);
	...
	case TNEGOP:
		...
		if(lt!=TYINT && lt!=TYREAL && lt!=TYLREAL && lt!=TYCOMPLEX)
			{
			exprerr("impossible unary + or - operation",0);
			goto err;
			}
		p->vtype = lt;
		break;
	...
	case TASGNOP:
		...
		if(lt==TYCHAR || rt==TYCHAR || lt==TYLOG || rt==TYLOG)
			...
			if(lt!=rt || l->vtypep->strsize!=r->vtypep->strsize
				...
				{
				exprerr("illegal structure assignment",0);
				goto err;
				}
			}
...
offsets(p);
if(v)	{
	v->blklevel = p->blklevel = blklevel;
	++ndecl[blklevel];
	v->varp = p;
	}
...
if(s==0 || (here && s!=thisctl) )
	{
	laberr("invalid case label location",0);
	return(0);
	}
...
if(here)
	if(q->labdefined)
		laberr("multiply defined case",0);
	else
		q->labdefined = 1;
return(q->labelno);
}
...
if(p->vsubs == 0)
	if(p->vdim==0 && p->vtype!=TYCHAR && p->vtype!=TYSTRUCT)
		{
		exprerr("need an aggregate to the left of arrow",0);
		frexpr(p);
		return( errnode() );
		}
	else	{
		if(p->vdim)
			...
			for(q = p->vdim->datap ; q ; q = q->nextp)
				s = mkchain( mkint(1), s);
			subscript(p, mknode(TLIST,0,s,0) );
			}
		}
...
if(p == 0)
	goto err;
a = coerce(TYREAL, p->datap);
p = p->nextp;
if(p->nextp)
	goto err;
b = coerce(TYREAL, p->datap);
...
if(a->tag!=TCONST || a->vtype!=TYREAL ||
   b->tag!=TCONST || b->vtype!=TYREAL )
		goto err;
...
mksub1()
{
return( mknode(TLIST,0, mkchain(mkint(1),0), 0) );
}
