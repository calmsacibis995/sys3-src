...
case TNOTOP:
	...
	case TLOGOP:	/* de Morgan's Law */
		...
		lp->leftp = mknode(TNOTOP,OPNOT,lp->leftp,0);
		lp->rightp=mknode(TNOTOP,OPNOT,lp->rightp,0);
		frexpblock(e);
		e = lp;
		goto top;
	...
	case TTEMP:
		if(t == LVAL)
			e = simple(LVAL,
			      mknode(TASGNOP,0, gentemp(e->leftp),e->leftp));
		break;
...
case TLOGOP: switch(subtype) {
		...
		case OP2AND:
			...
			if(lp->tag != TTEMP)
				lp = simple(RVAL, mknode(TASGNOP,0, gent(TYLOG,0),lp));
	...
	if(subtype==OP2OR || subtype==OP2AND)
		...
		if(e->vtype == TYCHAR)
			...
			e1 = mkchain(cpexpr(lp),mkchain(rp,
				mkchain(mkint(lp->vtypep->strsize),0)));
			excall(mkcall(mkftnblock(TYSUBR,"ef1ass"), mknode(TLIST,0, e1,0) ));
			}
...
typerr:
	exprerr("type match error", 0);
	return(e);

makesub:
	if(t==SUBVAL && e->vtype!=TYINT)
		warn1("Line %d. Non-integer subscript", yylineno);
	return( simple(RVAL, mknode(TASGNOP,0,gent(TYINT,0),e)) );
}

fold(e)
register ptr e;
...
switch(e->tag)
	...
	case TAROP:
		...
		if(c >= 0)
			return( mkint(c) );
		else	return(mknode(TNEGOP,OPMINUS, mkint(-c), 0) );
...
coerce(t,e)	/* coerce expression  e  to type  t */
int t;
register ptr e;
{
register int et;
...
bgnexec();
if(p->subtype == OP2OR)
	q = mknode(TNOTOP,OPNOT, cpexpr(p->leftp),0);
