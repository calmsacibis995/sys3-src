...
switch(p->tag)
	...
	case TLIST:
		...
		for(q = p->leftp ; q ; q = q->nextp)
			q1 = q1->nextp = mkchain( cpexpr(q->datap), 0);
		break;
	...
	default:
		fatal1("cpexpr: improper block tag %d", p->tag);
	}
return(e);
}
...
procnm()	/* name of the current procedure */
{
...
arg1(a)		/* make an argument list of one value */
ptr a;
{
return( mknode(TLIST,0, mkchain(a,0), 0) );
}



arg2(a,b)	/* make an argumentlist (a,b) */
ptr a,b;
{
register ptr p;

p = mkchain(a, mkchain(b,0));
...
arg4(a,b)	/* make an argument list of  (a,len(a), b,len(b)) */
ptr a,b;
{
register ptr p;
p = mkchain(b, mkchain(cpexpr(b->vtypep), 0));
