...
if(p->vtype==TYCHAR && p->addrblock.varleng!=0)
	{
	frexpr(p->vleng);
...
for(i=0 ; i<nd ; ++i)
	...
	if( (q = dims[i].ub) == NULL)
		...
		if( ISCONST(q) )
			{
			p->dims[i].dimsize = q;
			p->dims[i].dimexpr = PNULL;
			}
		else	{
			p->dims[i].dimsize = autovar(1, tyint, PNULL);
			p->dims[i].dimexpr = q;
			}
		if(p->nelt)
...
if( ISCONST(q) )
	...
	{
	p->baseoffset = autovar(1, tyint, PNULL);
	p->basexpr = q;
	}
}
