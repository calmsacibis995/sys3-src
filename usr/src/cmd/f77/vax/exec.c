...
settype(name, TYSUBR, PNULL);
p = mkfunct( mkprim(name, args, PNULL, PNULL) );
...
dovarp = mklhs( mkprim(np, PNULL, PNULL, PNULL) );
...
if( ISCONST(DOLIMIT) )
	...
	ctlstack->domax = mktemp(dotype, PNULL);
...
if( ISCONST(DOINCR) )
	...
	{
	ctlstack->dostep = mktemp(dotype, PNULL);
...
while(here == dorange)
	...
	if(np = ctlstack->donamep)
		{
		t = mkexpr(OPPLUSEQ,
			mklhs(mkprim(ctlstack->donamep, PNULL, PNULL, PNULL)),
		...
		putlabel(ctlstack->endlabel);
		if(ap = memversion(np))
			puteq(ap, mklhs( mkprim(np, PNULL, PNULL, PNULL)) );
		for(i = 0 ; i < 4 ; ++i)
			ctlstack->ctlabels[i] = 0;
...
p = mklhs(mkprim(vname, PNULL, PNULL, PNULL));
...
p = mklhs( mkprim(labvar, PNULL, PNULL, PNULL) );
if( ! ISINT(p->vtype) )
