...
#include "defs"
#include "pccdefs"
struct Addrblock *putcall(), *putcxeq(), *putcx1(), *realpart(), *imagpart();
ftnint lencat();
...
LOCAL struct Addrblock *putcxeq(p)
...
switch(opcode)
	...
	case OPNEG:
		putassign( realpart(resp), mkexpr(OPNEG, realpart(lp), PNULL) );
		putassign( imagpart(resp), mkexpr(OPNEG, imagpart(lp), PNULL) );
		*ncommap += 2;
		break;
		...
		if(rtype < TYCOMPLEX)
			...
			if(opcode == OPPLUS)
				putassign( imagpart(resp), imagpart(rp) );
					...
					mkexpr(OPNEG, imagpart(rp), PNULL) );
			}
		else
...
offp = (p->memoffset ? cpexpr(p->memoffset) : (tagptr)NULL );
...
for(cp = arglist ; cp ; cp = cp->nextp)
	if(indir)
		...
		if( ISCHAR(q) && q->vclass!=CLPROC )
			{
			charsp = hookup(charsp,mkchain(cpexpr(q->vleng),CHNULL));
			n += 2;
			}
		else
			n += 1;
