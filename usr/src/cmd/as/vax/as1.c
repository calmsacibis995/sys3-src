#
char AS1[] = "@(#)as1.c 1.23 79/05/30 15:08:36";	/* sccs ident */
#include <stdio.h>
#include <signal.h>
#include <a.out.h>
#include "as.h"
...
#ifdef vax
	/* this destroys the entries, so it had better be last */
	while (--n>=0) {
		if (p->ptype) p->type=p->ptype;
		p->tag=p->other;
		p->index=p->desc;
		fwrite(&(p->name[0]),sizeof(p->name)+sizeof(p->type)+
			sizeof(p->tag)+sizeof(p->index)+sizeof(p->value),
		  1, f);
		p++;
	}
#else
...
char **argv;
{
	...
	while (argc > 1) {
		if (argv[1][0]=='-' && argv[1][1]=='o') {
			...
			if (c - outfile > 3
			  && (*(c-=2) == 'c' || *c == 's')
			  && *--c == '.') {
				yyerror ("output would overwrite %s", outfile);
				exit(1);
			}
			continue;
		}
	...
	hshtab=sbrk(nhash*sizeof(*hshtab));
	for (c=nhash;--c>=0;) hshtab[c]=0;
