	...
	register  int  c;
{
	...
	lseek(kmem, (long)SYM_VALUE(Buf) + c * sizeof bbuf, 0);
	...
	int  sw;
{
	...
	lseek(kmem, (long)SYM_VALUE(Buf) + c * sizeof bufhdr, 0);
	if(read(kmem, &bufhdr, sizeof bufhdr) != sizeof bufhdr) {
		error("buffer header read error");
		return;
	}
	lseek(kmem, (unsigned)bufhdr.b_un.b_addr & ~HIGHBIT, 0);
	...
	case OCTAL:
		for(i=0, ip=(int *) buffer; ip != (int *) buffer[512]; i++, ip++) {
			if(i % 8 == 0)
				printf("\n%5.5o:\t", i+i);
	...
	case CHAR:
	case BYTE:
		for(i=0, cp = buffer; cp != &buffer[512]; i++, cp++) {
			if(i % (sw == CHAR ? 16 : 8) == 0)
				printf("\n%5.5o:\t", i);
			if(sw == CHAR) putch(*cp);
	...
	case INODE:
		for(i=1, dip = (struct dinode *) buffer; dip != (struct dinode *) buffer[512]; i++, dip++) {
	...
	case DIRECT:
		printf("\n");
		for(i=0, dp =(struct direct *)  buffer; dp != (struct direct *) &buffer[512]; i++, dp++) {
			bad = 0;
