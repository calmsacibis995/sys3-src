	...
	register  int  c;
{
	...
	lseek(kmem, (long)Buf->n_value + c * sizeof bbuf, 0);
	...
	int  sw;
{
	...
	case OCTAL:
		for(i=0, ip=buffer; ip != &buffer[512]; i++, ip++) {
			if(i % 8 == 0)
				printf("\n%5.5o:\t", i+i);
	...
	case INODE:
		for(i=1, dip = buffer; dip != &buffer[512]; i++, dip++) {
	...
	case DIRECT:
		printf("\n");
		for(i=0, dp = buffer; dp != &buffer[512]; i++, dp++) {
			bad = 0;
