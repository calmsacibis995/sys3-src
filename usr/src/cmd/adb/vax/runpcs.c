...
runpcs(runmode,execsig)
{
    ...
    IF adrflg 
    THEN
	IF (bkpt=scanbkpt(userpc)) THEN bkpt->flag = BKPTSET; FI
        userpc = dot;
    FI
    printf("%s: running\n", symfil);
    ...
    WHILE --loopcnt>=0
    DO
        IF (bkpt=scanbkpt(userpc)) ANDF (bkpt->flag == BKPTEXEC)
        THEN
            dot=bkpt->loc;
        ...
        THEN
            ...
            command(bkpt->comm,':');
            IF (bkpt->count == 0) 
	    THEN bkpt->count=bkpt->initcnt; rc=1; loopcnt = 0;
	    ELSE IF (runmode == CONTIN) THEN loopcnt++; FI
	    FI
...
doexec()
{
        ...
        *ap++=symfil;
        REP     IF rdc()==EOR THEN break; FI
                *ap = p;
                WHILE lastc!=EOR ANDF lastc!=SP ANDF lastc!=TB 
